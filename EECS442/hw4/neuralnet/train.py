...
def training_step(model, X_batch, y_batch, reg):
    ...
    ###########################################################################
    # loss, cache = None,None
    score,cache=model.forward(X_batch)
    data_loss, grad_score=softmax_loss(score,y_batch)
    regular_loss_W1,grad_W1=l2_regularization(model.fc1_W,reg)
    regular_loss_W2,grad_W2=l2_regularization(model.fc2_W,reg)
    loss=data_loss+regular_loss_W1+regular_loss_W2
    grads = model.backward(grad_score, cache)
    grads["fc1_W"]+=grad_W1
    grads["fc2_W"]+=grad_W2
    ###########################################################################
    #                             END OF YOUR CODE                            #
