...
def t4(R, X):
    ...
    """
    return np.vstack([np.dot(R, x) for x in X])
...
def t9(q, k, v):
    ...
    """
    return np.sum(np.exp(-np.sum((q-k)**2, axis=1, keepdims=True))*v)
...
def t10(Xs):
    ...
    C = []
    for m in Xs:
        C.append(np.sum(m, axis=0)/m.shape[0])
    R = np.zeros((len(Xs), len(Xs)))
    for i in range(len(Xs)):
        for j in range(i+1, len(Xs)):
            R[i, j] = R[j, i] = np.sqrt(np.sum((C[i]-C[j])**2))
    return R
...
def t11(X):
    ...
    D = (np.sum(X**2, axis=1, keepdims=True).T +
         np.sum(X**2, axis=1, keepdims=True))-2*np.dot(X, X.T)
    D[D < 0] = 0
    return np.sqrt(D)
...
def t12(X, Y):
    ...
    D = (np.sum(X**2, axis=1, keepdims=True) +
         np.sum(Y**2, axis=1, keepdims=True).T)-2*np.dot(X, Y.T)
    D[D < 0] = 0
    return np.sqrt(D)
...
def t18(N, r, x, y):
    ...
    I=np.zeros((N,N)).astype(dtype=float)
    I[(xv-x)**2+(yv-y)**2<r**2]=1
    return I
...
def t19(N, s, x, y):
    ...
    """
    xv , yv= np.meshgrid(np.arange(N).astype(dtype=float),np.arange(N).astype(dtype=float))
    return np.exp(-((xv-x)**2+(yv-y)**2)/s**2)
...
def t20(N, v):
    ...
    return np.abs(xv*v[0]+yv*v[1]+v[2])/np.sqrt(v[0]**2+v[1]**2)
