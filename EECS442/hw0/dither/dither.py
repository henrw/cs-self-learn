...
def resizeToSquare(I, maxDim):
    ...
    ratio=min(maxDim/I.shape[0],maxDim/I.shape[1])
    return cv2.resize(I,dsize=(int(I.shape[0]*ratio),int(I.shape[1]*ratio)))
...
def quantizeFloyd(IF, palette):
    ...
    """
    IF_cp=IF.copy()
    output = np.zeros(IF_cp.shape, dtype='uint8')
    for y in range(IF_cp.shape[0]):
        for x in range(IF_cp.shape[1]):
            oldValue = IF_cp[y,x]
            colorIndex = quantize(oldValue, palette)
            output[y][x] = colorIndex
            newValue = palette[colorIndex]
            error = oldValue-newValue
            if(y+1<IF_cp.shape[0]):
                IF_cp[y+1][x] += error*7/16
            if(y>0 and x+1<IF_cp.shape[1]):
                IF_cp[y-1][x+1] += error*3/16
            if(x+1<IF_cp.shape[1]):
                IF_cp[y][x+1] += error*5/16
            if(x+1<IF_cp.shape[1] and y+1<IF_cp.shape[0]):
                IF_cp[y+1][x+1] += error*1/16
    return output
